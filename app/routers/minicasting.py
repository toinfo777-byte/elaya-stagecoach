...
from app.storage.repo_extras import save_casting_session, save_feedback  # есть у тебя

mc_router = Router(name="minicasting")
router = mc_router  # совместимость с существующим импортом r_minicasting.router
...
async def start_minicasting(message_or_cq: Message | CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await m.answer("🎭 Мини-кастинг: короткий чек — пауза, тембр, ощущение. "
                   "На выходе — 1 рекомендация и круг тренировки.")
    await m.answer("Это мини-кастинг: 2–3 мин. Отвечай коротко. Готов?",
                   reply_markup=kb_mc_start())

# старое имя на всякий случай
minicasting_entry = start_minicasting

# ── быстрый ACK для всех mc:* (убираем «крутилку») ───────────────────────────
@mc_router.callback_query(F.data.startswith("mc:"))
async def _ack_any_mc(cq: CallbackQuery):
...
async def mc_summary(cq: CallbackQuery, state: FSMContext):
    ...
    tip = "Точка роста: не давай паузе проваливаться." if data.get("s1") == "no" else \
          "Точка роста: удерживай тембр после паузы."
    await cq.message.answer(f"Итог: {tip}\nСделай 1 круг тренировки и вернись.")
    await cq.message.answer("Оцени опыт 🔥/👌/😐 и добавь 1 слово-ощущение (необязательно).",
                            reply_markup=kb_mc_rate())

    # пробуем зафиксировать саму сессию (не критично)
    try:
...
async def mc_rate(cq: CallbackQuery, state: FSMContext):
    ...
    if rate == "skip":
        # сохраняем только emoji=None
        try:
            await save_feedback(cq.from_user.id, emoji=None, phrase=None)
        except Exception:
            pass
        await state.clear()
    ...
    await cq.message.answer("Принял эмодзи. Можешь одним словом дописать ощущение (до 140 симв) "
                            "или напиши «/menu».")
