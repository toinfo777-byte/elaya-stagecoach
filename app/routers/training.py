...
# ── one-time подсказка в рамках сессии ────────────────────────────────────────
TRAIN_SEEN_FLAG = "seen_info_training"

def _kb_training_levels():
...
# ── публичные входы (единые имена) ───────────────────────────────────────────
async def training_start(message: Message, state: FSMContext | None = None) -> None:
    if state is not None:
        # не чистим глобально, чтобы не сбить другие ветки; просто показываем уровни
        data = await state.get_data()
        if not data.get(TRAIN_SEEN_FLAG):
            await message.answer(
                "ℹ️ Тренировка дня: три готовых уровня. Выбирай время — получай шаги. "
                "После круга жми «✅ Выполнил(а)» — копим стрик."
            )
            await state.update_data(**{TRAIN_SEEN_FLAG: True})
    await message.answer(
        "🏋️ Тренировка дня\n\nВыбери уровень. После выполнения нажми «✅ Выполнил(а)».",
        reply_markup=_kb_training_levels(),
    )
...
async def show_plan(cb: CallbackQuery):
    ...
    await cb.message.answer(DETAILS[key], reply_markup=_kb_done_menu())

# ── завершение ────────────────────────────────────────────────────────────────
@router.callback_query(F.data == "tr:done")
async def training_done(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    # TODO: при желании — сохранить «день засчитан» в БД
