...
from app.routers.onboarding import router as onboarding_router     # /start (анкета)
...
# ⚠️ СТАРЫЙ фидбек может отсутствовать — подключаем опционально
try:
    from app.routers.feedback_old import router as feedback_router
except Exception:
...
# НОВОЕ: универсальный обработчик отзывов (кнопки 🔥/👌/😐 + «1 фраза»)
from app.bot.handlers.feedback import router as feedback2_router

# НОВОЕ: диагностический роутер — шлёт всё в логи
...
async def main():
    ...
    for r in (
        smoke_router,
        apply_router,
        deeplink_router,     # ⬅️ диплинки обрабатываем рано
        shortcuts_router,    # сквозные /training, /casting и кнопки
        reply_shortcuts_router,
        onboarding_router,   # анкета /start
        coach_router,
        training_router,
        casting_router,
        progress_router,

        feedback2_router,    # универсальный фидбек (кнопки/фраза)
        *( (feedback_router,) if feedback_router else () ),
        ...
        metrics_router,
        cancel_router,
        debug_router,        # диагностика до меню
